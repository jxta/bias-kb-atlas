...
記録層 908 ノード（2026-09-21、commit `e1faddf4`）中 100 ノード（関数体 census 線と Q8 の W 層）。抜き出し規則は生成器
`kb_atlas.py --profile public-grounding`（private 側 `knowledge/tools/`）に書かれており、手では選んでいません。
抜粋は規則で「接地の鎖」に固定しているので研究が進んでも増えませんが、背後の記録層は成長しています（853 → 903 → 908 ノード、9/6 → 9/15 → 9/21）。
その規模は**数だけ**を Atlas の案内タブ「記録層の規模」と `llms.txt` に出しています（型別・主張の状態・接地 151/151・双方向 168/184・k4 9/9・日ごとの増分）。
...
- 更新: 2026-09-03 接地ビュー／09-04 案内ビュー／09-06 ノード単体ページ・JSON-LD・スタブ・ツアー・現在地ページ／09-15 記録層 903 ノードに同期、英語 UI、記録層の規模、状態履歴（`status_history`）の表示、JSON-LD context を記録層の最新（82 語、来歴を W3C PROV へ）に、RO-Crate、ノートブック、CI 検査 `check_site.py`／09-23 記録層 908 ノードに同期、「地図」タブ（研究線ごとの節目・現在地・次の一手・方向層・直近の変化。非公開の線は数だけ）。
