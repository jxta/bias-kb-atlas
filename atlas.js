...
(function () {
  ...
  function viewStory(stage) {
    ...
    const disc = L.thread ? `<a class="iconbtn small" href="${esc(L.thread)}" target="_blank" rel="noopener">${T.map.discuss} ↗</a>` : META.discussions ? `<a class="iconbtn small" href="${esc(META.discussions)}${META.discussions.includes("?") ? "&" : "?"}discussions_q=${encodeURIComponent(L.short)}" target="_blank" rel="noopener">${T.map.discuss} ↗</a>` : `<span class="muted small" title="${T.map.discuss_none}">${T.map.discuss}: —</span>`;
